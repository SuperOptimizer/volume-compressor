...
For the three centre planes of every cube: the raw slice in grey with a red overlay whose opacity is the absolute error of the plain decode at that pixel with a fixed 4x gain (min(1, 4 |raw - decoded| / 255): an error of 32 shows as 50 % red, 64 or more as solid red), magnified 2x by pixel replication to 1024^2. `tools/diff_images.py`.

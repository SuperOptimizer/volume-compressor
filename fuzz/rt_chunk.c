 ...
 * must succeed, and every block decode must equal the full decode. (The lossy
 * format guarantees no error bound; hostile content can ring arbitrarily. At
 * q = VOLCOMP_Q_LOSSLESS the decode must equal the source byte for byte.) */
#include "../volcomp.h"
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  if (size < 2) return 0;
  /* data[0] == 0 exercises the lossless mode, 1..255 the lossy quantiser */
  float q = data[0] == 0 ? VOLCOMP_Q_LOSSLESS : 1.0f + (float)(data[0] - 1) * (254.0f / 254.0f);
  ...
  if (q == VOLCOMP_Q_LOSSLESS) {
    if (n > sizeof src + 8) abort();
    if (memcmp(src, dec, sizeof src) != 0) abort();
  }

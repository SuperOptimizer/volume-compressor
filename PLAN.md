...
| **Distortion metric (for RC)** | (a) plain Parseval-MSE estimate, (b) **HF-weighted / content-adaptive MSE** [leading default], (c) **true perceptual-in-loop** (GMSD/edge on decoded block) | **TEST ALL 3.** Parseval "exact" is NOT exact for integer-DCT (non-orthonormal) + dead-zone; worst at 5–20×. (c) needs the decoded spatial block → kills the free estimate → slower; build it & measure the speed cost — worth it iff fast enough. User observed **smear+blocking esp. in downscaled c3d chunks** on real data → bake-off must reproduce & gate that artifact with **GMSD** (catches blocking/smear PSNR hides). |
...
| ~~Scalability mode (quality/SNR LOD)~~ | **DROPPED** (2026-06-02) | No SNR layers / bit-plane scalability / nibble split. The **resolution pyramid `0/`..`7/` already serves progressive quality** in interactive zoom (5/→0/ IS progressive refinement). c3d's in-chunk LODs failed on *screen-coverage geometry*: serving level 3/ needs many 3/ chunks to span the screen, so coarse levels MUST be physically separate, independently-fetchable arrays with their own chunk grid (= why OME uses separate zarrs; our archive uses separate members — correct). **Quality = one knob: per-level target ratio** ("need higher quality → turn down ratio"). Two orthogonal simple mechanisms, not three entangled ones. |
